(function () {
  ...
  Spider.prototype.power = function (impulse) {
    ...
    for (var i = 0; i < 2; i++) {
      if (this.vel[i] > this.max_vel) {
        this.vel[i] = this.max_vel;
      } else if (this.vel[i] < -this.max_vel) {
        this.vel[i] = -this.max_vel;
      }
    }
  };
  ...
  Spider.prototype.turn = function () {
    amt = Math.min(Math.abs(this.angleToShip() - this.dir), Math.PI / 8);
    this.dir += this.angleToShip() > this.dir ? amt : -amt;
  };

  Spider.prototype.accelerate = function () {
    var velX = 0.05 * Math.cos(this.dir);
    var velY = 0.05 * Math.sin(this.dir);
