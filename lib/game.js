(function () {
  ...
  Game.prototype.addSpider = function () {
    ...
    var yPos = Math.random() * ((this.yEnd - this.yStart) * 0.75);
  ...
  Game.prototype.addPowerup = function (pos) {
    ...
    var selection = Math.floor(Math.random() * Object.keys(types).length);
    var attr = Object.keys(types)[selection];
    ...
    this.powerups.push(new Centipede.Powerup({
      ...
      pos: pos,
      vel: [
        Math.random() * (2 + this.level * 0.25),
        Math.random() * (2 + this.level * 0.25) - (1 + this.level * 0.125)
      ],
      attribute: attr,
      effect: effect,
      color: color
